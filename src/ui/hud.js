...
export class HUD {
	constructor() {
		...
		this.radarAltElem = document.getElementById('radar-altitude');
		this.verticalSpeedElem = document.getElementById('vertical-speed');
		this.flightAssistElem = document.getElementById('flight-assist-status');
		this.bestScoreElem = document.getElementById('hud-best-score');
	...
	updatePauseMenu(state, currentRegionName, npcs = []) {
		...
		if (this.pauseLatElem) {
			...
			this.pauseLatElem.innerText = `${Math.abs(state.lat).toFixed(4)} deg ${latDir}`;
		}
		if (this.pauseLonElem) {
			const lonDir = state.lon >= 0 ? 'E' : 'W';
			this.pauseLonElem.innerText = `${Math.abs(state.lon).toFixed(4)} deg ${lonDir}`;
		}
	...
	update(state, npcs = []) {
		...
		if (this.radarAltElem) {
			if (typeof state.radarAltitude === 'number') {
				const radarAltFeet = Math.max(0, Math.round(state.radarAltitude * 3.28084));
				this.radarAltElem.innerText = radarAltFeet.toString().padStart(5, '0');
			} else {
				this.radarAltElem.innerText = '-----';
			}
		}

		if (this.verticalSpeedElem) {
			if (typeof state.verticalSpeedFpm === 'number') {
				const vs = Math.round(state.verticalSpeedFpm);
				const sign = vs > 0 ? '+' : '';
				this.verticalSpeedElem.innerText = `${sign}${vs}`;
			} else {
				this.verticalSpeedElem.innerText = '-----';
			}
		}

		if (this.flightAssistElem) {
			this.flightAssistElem.innerText = state.flightAssistEnabled ? 'ASSIST' : 'MANUAL';
			this.flightAssistElem.style.color = state.flightAssistEnabled ? '#0f0' : '#ffd166';
		}

		if (this.bestScoreElem) {
			this.bestScoreElem.innerText = (state.bestScore || 0).toString().padStart(6, '0');
		}
		...
		if (this.coordsElem) {
			const latDir = state.lat >= 0 ? 'N' : 'S';
			const lonDir = state.lon >= 0 ? 'E' : 'W';
			this.coordsElem.innerText = `POS: ${Math.abs(state.lat).toFixed(4)} deg ${latDir} ${Math.abs(state.lon).toFixed(4)} deg ${lonDir}`;
		}
