export class PlaneController {
	...
	update(dt = 1 / 60) {
		...
		this.input.toggleFlightAssist = false;

		this.input.weaponIndex = -1;
		if (this.keys['1'] && !this.prevKeys['1']) this.input.weaponIndex = 0;
		if (this.keys['2'] && !this.prevKeys['2']) this.input.weaponIndex = 1;

		const accelRate = 0.5;
		const throttleDelta = accelRate * dt;
		if (this.keys['w']) {
			this.input.throttle = Math.min(1, this.input.throttle + throttleDelta);
		} else if (this.keys['s']) {
			this.input.throttle = Math.max(0, this.input.throttle - throttleDelta);
		}
	...
	reset() {
		...
		this.input.roll = 0;
		this.input.yaw = 0;
		this.input.flightAssistEnabled = false;
		this.input.toggleFlightAssist = false;
	}
